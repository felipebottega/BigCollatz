...
The adaptive runner uses the single evaluator architecture in `bigcollatz.evaluator`: `evaluate()` and `evaluate_with_metrics()` both delegate to one private exact trajectory engine for the Collatz transition, first-seen map, repeated-at index, cycle length, stopping at `1`, maximum tracking, and `EvaluationResult` construction. Metric collection is optional and cannot change the `EvaluationResult`.

Recurrence metrics are heuristic only. The implemented metrics are: odd-step count and density; first descent step; exact maximum-excursion fraction; same-decimal-digit-band return count; repeated residue hits in a bounded window; and the residue modulus. Per-cell aggregation records means for count/density/return/residue-hit values, mean first descent over defined values plus undefined count, and the maximum exact excursion fraction. These metrics are used only in deterministic scoring, never as evidence for cycle existence.

Canonical cycle reconstruction replays states through `repeated_at_step`, slices `states[first_seen_step:repeated_at_step]`, requires the first member to equal `repeated_integer`, verifies member count equals `cycle_length`, verifies every transition, and verifies final closure. Independent verification replays the starting integer with the authoritative evaluator, reconstructs the cycle again, compares scalar evidence and ordered members, rejects cycles containing `1`, and validates canonical decimal strings.

Discovery artifacts are complete and unabridged. A repeated state is first persisted to `results/cycle_candidates.json`. A verified nontrivial cycle additionally writes `results/nontrivial_cycle_discovery.json` and `NONTRIVIAL_CYCLE_FOUND.md` with full starting integer, repeated integer, indices, cycle members, pilot or experiment identifier, strategy, seed, cell, family, generation parameters, source metadata, validation mode, and independent replay confirmation. After verified discovery, the adaptive runner stops immediately and writes `results/<pilot_id>/summary.json` with `stopped_early=true` and `stopping_reason="verified_nontrivial_cycle"`.

Adaptive cells explicitly contain `cell_id`, `family`, `strategy`, `validation_mode`, `source_parent`, `parent_rank`, `candidate_count`, and complete family parameters. Family bindings are strict: parity-prefix uses `S1-parity-prefix-top10` and `parity_prefix`; decimal-suffix uses `S5-decimal-suffix-top10` and `decimal_suffix`; residue uses `S6-residue-class-top10` and `residue`. CandidateRecord metadata must exactly equal cell metadata before mathematical validation: no missing fields, no unrelated family metadata, and no record-to-cell substitutions are allowed.

Adaptive pilots use one pilot-wide `seen_candidates` set, so duplicate candidates across cells fail before a second evaluator call. The pilot top tail is exactly `ceil(10% of completed trajectories)`, sorted by trajectory length descending with ties broken by `cell_id` and cell-local order. The deterministic score uses persisted summary fields only: mean length + 0.6*p90 + 0.3*p99 (or p90 fallback) + 25*top-tail count + 10*mean repeated-residue hits + 50*repeated-state count. Stage B allocation is deterministic largest-remainder allocation over nonnegative scores, gives every selected cell a positive quota when feasible, handles all-zero scores uniformly, and rejects infeasible minimum quotas.

Current pilot identifiers: Stage A `p007-adaptive-stage-a-300`; Stage B `p008-adaptive-stage-b-300`. Both are isolated pilots and did not update `results/global_top_10.json`.
