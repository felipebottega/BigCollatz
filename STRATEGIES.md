...
## P008 stage-B adaptive selection procedure

Pilot `p008-adaptive-stage-b-400` is the second-stage adaptive pilot after P007. It promotes exactly three P007 sustained-tail cells—rank-2 decimal suffix with 64 preserved digits, rank-2 parity prefix length 256, and rank-2 parity prefix length 128—and retains the rank-1 residue modulo `2**128 + 1` cell only as a reproducibility sentinel for P007's isolated residue maximum. Each cell receives 100 deterministic candidates, increasing per-cell sample size from P007's 25 while remaining a pilot rather than a full experiment.

P008 reconstructs source parents from `results/global_top_10.json`, derives each cell seed from `p008-adaptive-stage-b-400/main/2026-08-05/<cell_id>`, validates complete `CandidateRecord` provenance and family invariants before evaluation, reconstructs the deterministic P007 candidate set, and rejects any P008 candidate that reappears in P007 before calling the evaluator. Ranking uses trajectory length descending, then cell id, candidate order within cell, and starting integer; the persisted P008 ranking is the top 40 records, representing the overall top 10% tail.
