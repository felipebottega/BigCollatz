...
from bigcollatz.p007 import (
    build_p007_cells,
    build_p007_generators,
    CANDIDATES_PER_CELL,
    DETERMINISTIC_SEED,
)
...
def global_top(path: Path):
    ...
    records = [
        {"starting_integer": str(PARENT1), "total_unaccelerated_trajectory_length": 10},
        {"starting_integer": str(PARENT2), "total_unaccelerated_trajectory_length": 9},
    ]
    for i in range(8):
        records.append(
            {
                "starting_integer": str(3 + i if i < 7 else 11).ljust(1000, "0"),
                "total_unaccelerated_trajectory_length": 8 - i,
            }
        )
...
class P007RankingTests(unittest.TestCase):
    def test_top30_order_ties_and_decimal_strings(self):
        ...
        for i in range(35):
            trajectories.append(
                {
                    "starting_integer": str(10**999 + i),
                    "trajectory_length": 100 if i in (1, 2) else i,
                    "maximum_integer": str(10**1000 + i),
                    "cell_id": "b" if i == 1 else "a",
                    "family": "f",
                    "strategy": "s",
                    "source_parent": str(PARENT1),
                    "parent_rank": 1,
                    "generation_parameters": {"x": 1},
                    "validation_mode": "v",
                    "deterministic_seed": DETERMINISTIC_SEED,
                    "candidate_order_within_cell": i,
                }
            )
    ...
    def test_summary_no_placeholders_and_global_unchanged(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            ev = Mock(
                return_value=(
                    EvaluationResult(
                        rec.candidate, 7, "reached_one", rec.candidate * 3
                    ),
                    EvaluationMetrics(
                        3, (3, 7), 2, rec.candidate * 3, rec.candidate, 5, 6, 1024
                    ),
                )
            )
