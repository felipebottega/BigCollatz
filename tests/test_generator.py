...
class GeneratorTests(unittest.TestCase):
    def test_deterministic_distinct_and_at_least_1001_digits(self):
        first = list(baseline_candidates(10_000, "fixture"))
        self.assertEqual(len(first), len(set(first)))
        self.assertTrue(all(len(str(value)) == 1001 for value in first))
    ...
    def test_seed_selects_stream(self):
        self.assertNotEqual(
            list(baseline_candidates(3, "a")), list(baseline_candidates(3, "b"))
        )
    ...
    def test_candidate_size_is_unbounded_above_fixed_minimum(self):
        for invalid in (1000, 0, -1, 1001.0, True):
            with self.assertRaises(ValueError):
                candidate_interval(invalid)  # type: ignore[arg-type]
        candidates = list(baseline_candidates(3, "large", decimal_digits=5000))
        self.assertTrue(
            all(len(decimal_string(value)) == 5000 for value in candidates)
        )
...
class ParityPrefixGeneratorTests(unittest.TestCase):
    ...
    def test_properties_and_parity_prefix(self):
        ...
        self.assertTrue(all(len(str(candidate)) == 1001 for candidate in candidates))
        ...
        self.assertTrue(
            all(
                validate_parity_prefix(candidate, parent, 256)
...
class WeightedLineageGeneratorTests(unittest.TestCase):
    def test_e002_style_weights_allocate_10000_exactly(self):
        self.assertEqual(
            weighted_allocation(10_000, [3, 3, 2, 1, 1]), [3000, 3000, 2000, 1000, 1000]
        )

    def test_e003_style_weights_allocate_10000_exactly(self):
        self.assertEqual(
            weighted_allocation(10_000, [4, 4, 1, 1]), [4000, 4000, 1000, 1000]
        )
    ...
    def test_load_lineage_weights_uses_only_productive_lineages_in_source_order(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                load_lineage_weights(path), [(int(parent_b), 2), (int(parent_a), 1)]
            )
    ...
    def test_s3_lineage_weights_are_derived_and_validated(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                weighted_allocation(10_000, [weight for _, weight in weights]),
                [4000, 4000, 1000, 1000],
            )
    ...
    def test_invalid_lineage_source_files_have_clear_errors(self):
        ...
        cases = [
            ...
            (
                json.dumps(
                    [{"parent_starting_integer": "0" + "1" * 999, "prefix_length": 256}]
                ),
                "canonical",
            ),
            (
    ...
    def test_mixed_prefix_records_are_distinct_allowed_size_and_validate(self):
        ...
        self.assertTrue(all(len(str(candidate)) == 1001 for candidate in starts))
...
class ExplicitRecordGeneratorTests(unittest.TestCase):
    def test_decimal_suffix_records_are_explicit_distinct_and_validate(self):
        ...
        self.assertTrue(all(len(str(r.candidate)) == 1001 for r in records))
        self.assertTrue(
            all(
        ...
        self.assertTrue(
            all(
                validate_decimal_suffix(r.candidate, r.parent, r.suffix_digits)
                for r in records
            )
        )
    ...
    def test_residue_records_are_explicit_distinct_and_validate(self):
        ...
        self.assertTrue(
            all(
                validate_residue(r.candidate, r.residue_modulus, r.residue)
                for r in records
            )
        )
