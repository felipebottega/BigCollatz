...
def metric():
    return EvaluationMetrics(1, (1, 1), None, 1, 1, 0, 0, 1024)


def reached(n):
    return EvaluationResult(n, 2, "reached_one", n), metric()


def cell(f="parity-prefix", count=1):
    if f == "parity-prefix":
        return AdaptiveCell(
            "c", f, S1_STRATEGY, "parity_prefix", P, 1, count, {"prefix_length": 1}
        )
    if f == "decimal-suffix":
        return AdaptiveCell(
            "c", f, S5_STRATEGY, "decimal_suffix", P, 1, count, {"suffix_digits": 1}
        )
    return AdaptiveCell(
        "c", f, S6_STRATEGY, "residue", P, 1, count, {"residue_modulus": 10}
    )
...
def rec(f="parity-prefix", cand=C):
    if f == "parity-prefix":
        return CandidateRecord(
            cand, S1_STRATEGY, "parity_prefix", parent=P, prefix_length=1
        )
    if f == "decimal-suffix":
        return CandidateRecord(
            ((C // 10) * 10 + P % 10) + 10,
...
class AdaptiveTests(unittest.TestCase):
    def test_cell_validation(self):
        for kwargs in [{"parent_rank": 0}, {"parent_rank": -1}, {"candidate_count": 0}]:
            ...
            c = AdaptiveCell(
                ...
                c.validation_mode,
                c.source_parent,
                kwargs.get("parent_rank", c.parent_rank),
                kwargs.get("candidate_count", c.candidate_count),
                c.parameters,
            )
        ...
        with self.assertRaises(ValueError):
            validate_cell(
                AdaptiveCell(
                    "c", "parity-prefix", S1_STRATEGY, "parity_prefix", P, 1, 1, {}
                )
            )
        with self.assertRaises(ValueError):
            validate_cell(
    ...
    def test_record_positive_controls(self):
        for f in ("parity-prefix", "decimal-suffix", "residue"):
            seen = set()
            validate_record_for_cell(rec(f), cell(f), seen)
            self.assertEqual(len(seen), 1)

    def test_record_rejections_before_eval(self):
        cases = [
            CandidateRecord(C, S1_STRATEGY, "parity_prefix", parent=P),
            CandidateRecord(C, S1_STRATEGY, "parity_prefix", parent=P, prefix_length=2),
            CandidateRecord(
                C,
                S1_STRATEGY,
                "parity_prefix",
            ...
            CandidateRecord(
                C, S1_STRATEGY, "parity_prefix", parent=P, prefix_length=1, residue=1
            ),
            CandidateRecord(C, "wrong", "parity_prefix", parent=P, prefix_length=1),
            CandidateRecord(C, S1_STRATEGY, "wrong", parent=P, prefix_length=1),
            CandidateRecord(
                C, S1_STRATEGY, "parity_prefix", parent=P + 2, prefix_length=1
            ),
        ]
        for r in cases:
            with self.subTest(r=r), self.assertRaises(ValueError):
                validate_record_for_cell(r, cell(), set())

    def test_generator_counts_timing_and_totals(self):
        times = iter([0, 0, 2, 2])
        with tempfile.TemporaryDirectory() as d:
            s = run_adaptive_pilot(
                Path(d),
                pilot_id="p",
            ...
            self.assertEqual(s["distinct_candidate_count"], 1)
            self.assertEqual(s["cells"][0]["runtime_seconds"], 2)
            self.assertEqual(s["cells"][0]["trajectories_per_second"], 0.5)
    ...
    def test_pilot_id_must_be_single_portable_path_safe_name(self):
        ...
        for pilot_id in invalid_pilot_ids:
            with (
                self.subTest(pilot_id=pilot_id),
                tempfile.TemporaryDirectory() as d,
                self.assertRaisesRegex(ValueError, "pilot_id"),
            ):
                run_adaptive_pilot(
                    Path(d),
                    pilot_id=pilot_id,
    ...
    def test_early_stop(self):
        ...
        with (
            tempfile.TemporaryDirectory() as d,
            patch("bigcollatz.adaptive.reconstruct_cycle", return_value=[C]),
            patch("bigcollatz.adaptive.verify_nontrivial_cycle") as ver,
            patch("bigcollatz.cycle.verify_nontrivial_cycle") as ver2,
        ):
            ver.return_value = type(
                "V", (), {"confirmed": True, "failure_reason": None}
            )()
            ver2.return_value = type(
                "V", (), {"confirmed": True, "members": [C], "failure_reason": None}
            )()
            s = run_adaptive_pilot(
                Path(d),
                pilot_id="p",
            ...
            self.assertEqual(s["stopping_reason"], "verified_nontrivial_cycle")
            self.assertEqual(s["candidates_evaluated"], 1)
            self.assertTrue((Path(d) / "NONTRIVIAL_CYCLE_FOUND.md").exists())
...
class CandidateRecordCompleteNegativeTests(unittest.TestCase):
    ...
    def test_all_required_negative_metadata_cases(self):
        ...
        cases = [
            ...
            (
                CandidateRecord(
                    C, S5_STRATEGY, "decimal_suffix", parent=P, suffix_digits=2
                ),
                suffix_cell,
            ),
            (
            ...
            (
                CandidateRecord(C, S6_STRATEGY, "residue", parent=P, residue=P % 10),
                residue_cell,
            ),
            (
                CandidateRecord(
                    C, S6_STRATEGY, "residue", parent=P, residue_modulus=10
                ),
                residue_cell,
            ),
            (
            ...
            (
                CandidateRecord(
                    10**999 + 2, S1_STRATEGY, "parity_prefix", parent=P, prefix_length=1
                ),
                cell(),
            ),
        ]
    ...
    def test_multiple_cells_exact_counts(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            s = run_adaptive_pilot(
                ...
                deterministic_seed="seed",
                cells=[c1, c2],
                generators={"c": [rec()], "d": [rec("decimal-suffix")]},
                evaluator=reached,
            )
...
class AdaptiveSummaryAccuracyTests(unittest.TestCase):
    def test_normal_completion_aggregates_thresholds_tail_and_score(self):
        ...
        results = [
            (
                EvaluationResult(C, 25000, "reached_one", C * 2),
                EvaluationMetrics(3, (3, 10), 5, 4, 2, 1, 7, 1024),
            ),
            (
                EvaluationResult(C + 2, 27000, "reached_one", (C + 2) * 3),
                EvaluationMetrics(5, (5, 20), None, 9, 3, 3, 11, 1024),
            ),
        ]
        with tempfile.TemporaryDirectory() as d:
            s = run_adaptive_pilot(
                ...
                cells=[c],
                generators={"c": records},
                evaluator=Mock(side_effect=results),
            )
        cs = s["cells"][0]
        ...
        self.assertEqual(
            cs["fixed_threshold_exceedance_counts"],
            {"length_gte_25000": 2, "length_gte_26000": 1, "length_gte_27000": 1},
        )
    ...
    def test_top_tail_ties_are_deterministic_and_score_uses_metric(self):
        ...
        def run(hit):
            with tempfile.TemporaryDirectory() as d:
                return run_adaptive_pilot(
                    ...
                    deterministic_seed="seed",
                    cells=[c1, c2],
                    generators={"c": [rec(cand=C)], "d": [rec(cand=C + 2)]},
                    evaluator=Mock(
                        side_effect=[
                            (
                                EvaluationResult(C, 100, "reached_one", C),
                                EvaluationMetrics(1, (1, 2), 1, 1, 1, 0, hit, 1024),
                            ),
                            (
                                EvaluationResult(C + 2, 100, "reached_one", C + 2),
                                EvaluationMetrics(1, (1, 2), 1, 1, 1, 0, 0, 1024),
                            ),
                        ]
                    ),
                )
        ...
        s2 = run(5)
        self.assertEqual(
            [x["overall_pilot_top_tail_count"] for x in s1["cells"]], [1, 0]
        )
        self.assertGreater(
            s2["cells"][0]["deterministic_score"], s1["cells"][0]["deterministic_score"]
        )
    ...
    def test_inconsistent_residue_modulus_and_global_modification_are_errors(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertFalse(
                json.loads((root / "results" / "p" / "summary.json").read_text())[
                    "global_top_10_isolated"
                ]
            )
        with tempfile.TemporaryDirectory() as d:
            vals = [
                (
                    EvaluationResult(C, 1, "reached_one", C),
                    EvaluationMetrics(1, (1, 1), None, 1, 1, 0, 0, 1024),
                ),
                (
                    EvaluationResult(C + 2, 1, "reached_one", C + 2),
                    EvaluationMetrics(1, (1, 1), None, 1, 1, 0, 0, 2048),
                ),
            ]
            with self.assertRaisesRegex(ValueError, "inconsistent residue"):
                run_adaptive_pilot(
                    ...
                    deterministic_seed="seed",
                    cells=[cell(count=2)],
                    generators={"c": [rec(), rec(cand=C + 2)]},
                    evaluator=Mock(side_effect=vals),
                )
    ...
    def test_immediate_stop_does_not_advance_generator(self):
        ...
        with (
            ...
            self.assertEqual(s["stopping_reason"], "verified_nontrivial_cycle")
            self.assertTrue(
                (root / "results" / "nontrivial_cycle_discovery.json").exists()
            )
