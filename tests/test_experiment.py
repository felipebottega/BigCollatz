...
def interrupted_evaluate(candidate: int) -> EvaluationResult:
    return EvaluationResult(
        candidate, 123, "interrupted", candidate, stopping_reason="user_stop"
    )
...
class ExperimentTests(unittest.TestCase):
    ...
    def test_guided_strategy_records_generation_parameters(self, _evaluate):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                parameters["source_global_top_10_file"], "results/global_top_10.json"
            )
            ...
            self.assertEqual(
                [
                    ...
                    for item in parameters["allocation_per_parent"]
                ],
                [3, 2],
            )
    ...
    def test_writes_only_summaries_and_top_tens(self, _evaluate, _candidates):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                experiment_files, {"summary.json", "summary.md", "top_10.json"}
            )
            self.assertEqual(len(result["top_10"]), 10)
            self.assertEqual(
                result["top_10"][0]["total_unaccelerated_trajectory_length"], 11
            )
            ...
            self.assertFalse(
                any(path.suffix in {".jsonl", ".csv"} for path in root.rglob("*"))
            )
            self.assertTrue(
                all(
                    "parent_starting_integer" not in entry
                    and "prefix_length" not in entry
    ...
    def test_guided_winner_lineage_survives_outputs_and_global_merge(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source.parent.mkdir(parents=True)
            source.write_text(
                json.dumps([{"starting_integer": str(parent)} for parent in parents])
            )

            result = run_experiment(
                root,
                experiment_id="guided-lineage",
                count=4,
                seed="fixture",
                strategy=S1_STRATEGY,
                prefix_length=17,
            ...
            for entry in result["top_10"]:
                self.assertEqual(
                    entry["parent_starting_integer"],
                    expected_parents[entry["starting_integer"]],
                )
                self.assertEqual(entry["prefix_length"], 17)
            stored = json.loads(
                (root / "results/guided-lineage/top_10.json").read_text()
            )
            self.assertEqual(stored, result["top_10"])
            ...
            self.assertTrue(
                all(
                    "parent_starting_integer" in entry and entry["prefix_length"] == 17
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source.write_text(
                json.dumps(
                    [
                        {
                            "parent_starting_integer": str(parents[0]),
                            "prefix_length": 256,
                        },
                        {
                            "parent_starting_integer": str(parents[0]),
                        ...
                        {
                            "parent_starting_integer": str(parents[1]),
                            "prefix_length": 256,
                        },
                    ]
                )
            ...
            self.assertEqual(
                [item["weight"] for item in parameters["lineage_weights"]], [2, 1]
            )
            self.assertEqual(
                [
                    item["candidate_count"]
            ...
            self.assertEqual(stored, result["top_10"])
            persistent_global = json.loads(
                (root / "results/global_top_10.json").read_text()
            )
            self.assertTrue(
                all(
                    "parent_starting_integer" in entry and entry["prefix_length"] == 256
                    for entry in persistent_global
                )
            )
    ...
    @patch("bigcollatz.experiment.weighted_parity_prefix_candidate_records")
    @patch("bigcollatz.experiment.evaluate", side_effect=fake_evaluate)
    def test_s3_records_parameters_lineage_outputs_and_report(
        self, _evaluate, candidate_records
    ):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            source.write_text(
                json.dumps(
                    [
                        {
                            "parent_starting_integer": str(parent),
                            "prefix_length": 256,
                            "strategy": S2_STRATEGY,
                            "experiment_id": "e003-s2-weighted-lineages-256",
                            "outcome": "reached_one",
                        }
                        for parent in [parents[0]] * 4
                        + [parents[1]] * 4
                        + [parents[2], parents[3]]
                    ]
                )
            ...
            self.assertEqual(
                [item["weight"] for item in parameters["lineage_weights"]], [4, 4, 1, 1]
            )
            self.assertEqual(
                [
                    item["candidate_count"]
            ...
            candidate_records.assert_called_once_with(
                10_000,
                [(parents[0], 4), (parents[1], 4), (parents[2], 1), (parents[3], 1)],
                "fixture",
                256,
                S3_STRATEGY,
            )
            ...
            self.assertIn(
                "Parent (abbreviated)",
                (root / "results/s3-lineage/summary.md").read_text(),
            )

    @patch("bigcollatz.experiment.mixed_prefix_candidate_records")
    @patch("bigcollatz.experiment.evaluate", side_effect=fake_evaluate)
    def test_s4_records_mixed_prefix_lineage_outputs_and_report(
        self, _evaluate, candidate_records
    ):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            self.assertEqual(
                [
                    item["candidate_count"]
                    for item in parameters["allocation_per_parent_prefix"]
                ],
                [1, 1, 1, 1, 1, 1],
            )
            self.assertTrue(
                all("parent_starting_integer" in entry for entry in result["top_10"])
            )
            self.assertEqual(
                sorted({entry["prefix_length"] for entry in result["top_10"]}),
                [128, 256, 384],
            )
            self.assertIn(
                "Parent (abbreviated)",
    ...
    def test_global_top_ten_is_merged_and_deduplicated(self, _evaluate, _candidates):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                len({entry["starting_integer"] for entry in global_top}), 10
            )
    ...
    def test_interrupted_are_excluded_from_statistics_and_top_tens(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                all(entry["outcome"] != "interrupted" for entry in result["top_10"])
            )
            self.assertTrue(
                all(
    ...
    @patch("bigcollatz.experiment.baseline_candidates", side_effect=fake_candidates)
    @patch("bigcollatz.experiment.evaluate", side_effect=interrupted_evaluate)
    def test_all_interrupted_has_null_statistics_and_empty_tops(
        self, _evaluate, _candidates
    ):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            self.assertEqual(
                json.loads((root / "results/global_top_10.json").read_text()), []
            )
...
class CycleCandidatePersistenceTests(unittest.TestCase):
    ...
    def test_repeated_state_count_and_persistence_outside_top_ten(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                all(entry["outcome"] == "reached_one" for entry in result["top_10"])
            )
    ...
    def test_s2_cycle_record_preserves_lineage(self, _evaluate, candidate_records):
        with tempfile.TemporaryDirectory() as directory:
            ...
            run_experiment(
                root, experiment_id="s2-cycles", count=1, strategy=S2_STRATEGY
            )
            record = json.loads((root / "results/cycle_candidates.json").read_text())[0]
            self.assertEqual(record["parent_starting_integer"], str(parent))
    ...
    def test_s3_cycle_record_preserves_lineage(self, _evaluate, candidate_records):
        with tempfile.TemporaryDirectory() as directory:
            ...
            run_experiment(
                root, experiment_id="s3-cycles", count=1, strategy=S3_STRATEGY
            )
            record = json.loads((root / "results/cycle_candidates.json").read_text())[0]
            self.assertEqual(record["parent_starting_integer"], str(parent))
            self.assertEqual(record["prefix_length"], 256)
    ...
    @patch("bigcollatz.experiment.baseline_candidates", side_effect=fake_candidates)
    @patch("bigcollatz.experiment.evaluate", side_effect=fake_evaluate)
    def test_no_repeated_state_does_not_create_or_modify_cycle_file(
        self, _evaluate, _candidates
    ):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
...
class StrategyBoundValidationTests(unittest.TestCase):
    ...
    @patch(
        "bigcollatz.experiment.validate_residue",
        side_effect=AssertionError("wrong validator"),
    )
    ...
    def test_s5_dispatch_metadata_cycles_and_unrelated_validator_not_called(
        self, records, _eval, dec_validator, res_validator
    ):
    ...
    @patch(
        "bigcollatz.experiment.validate_decimal_suffix",
        side_effect=AssertionError("wrong validator"),
    )
    @patch("bigcollatz.experiment.validate_residue", return_value=True)
    @patch("bigcollatz.experiment.evaluate", side_effect=duplicate_repeated_evaluate)
    @patch("bigcollatz.experiment.residue_candidate_records")
    def test_s6_dispatch_metadata_cycles_and_unrelated_validator_not_called(
        self, records, _eval, res_validator, dec_validator
    ):
    ...
    @patch("bigcollatz.experiment.evaluate", side_effect=fake_evaluate)
    @patch("bigcollatz.experiment.decimal_suffix_candidate_records")
    def test_s5_rejects_residue_mode_and_incomplete_or_invalid_metadata(
        self, records, _eval
    ):
        from bigcollatz.generator import CandidateRecord, S5_STRATEGY
    ...
    @patch("bigcollatz.experiment.evaluate", side_effect=fake_evaluate)
    @patch("bigcollatz.experiment.residue_candidate_records")
    def test_s6_rejects_decimal_suffix_mode_and_incomplete_or_invalid_metadata(
        self, records, _eval
    ):
        from bigcollatz.generator import CandidateRecord, S6_STRATEGY
