...
def reconstruct_cycle(
    start: int, result: EvaluationResult, *, transition: Transition = collatz_step
) -> list[int]:
...
def verify_cycle_members(
    ...
    *,
    ...
    if (
        result.repeated_state is None
        or result.first_seen_step is None
        or result.repeated_at_step is None
        or result.cycle_length is None
    ):
    ...
    if len(members) != result.repeated_at_step - result.first_seen_step:
        raise ValueError(
            "cycle member count does not match repeated-at minus first-seen"
        )
...
def verify_nontrivial_cycle(
    start: int,
    claimed_result: EvaluationResult,
    claimed_members: list[Any] | None = None,
    *,
    transition: Transition = collatz_step,
    ...
    try:
        ...
        if replay.outcome != "repeated_state":
            return CycleVerification(
                False, [], "independent replay did not find a repeated state"
            )
        ...
        if claimed_members is not None:
            ...
            for item in claimed_members:
                if not isinstance(item, str) or str(int(item)) != item:
                    return CycleVerification(
                        False, members, "noncanonical decimal evidence"
                    )
                canonical_claim.append(int(item))
            if canonical_claim != members:
                return CycleVerification(
                    False, members, "claimed cycle members mismatch"
                )
...
def write_discovery_artifacts(
    ...
    *,
    ...
    verification = verify_nontrivial_cycle(
        starting_integer, result, [str(v) for v in cycle_members]
    )
    if not verification.confirmed:
        raise ValueError(f"cycle verification failed: {verification.failure_reason}")
    compact = _cycle_candidate_record(
        result=result,
        candidate=starting_integer,
        experiment_id=pilot_id,
        strategy=strategy,
        metadata={"pilot_id": pilot_id, "cell_id": cell_id, "family": family},
    )
    _persist_cycle_candidates(output_root, [compact])
    payload = {
        "starting_integer": str(starting_integer),
        ...
        "cycle_members": [str(v) for v in verification.members],
        "pilot_id": pilot_id,
        "strategy": strategy,
    ...
    md_path.write_text(
        "\n".join(
            [
                "# Nontrivial Cycle Found",
                "",
                f"Starting integer: `{starting_integer}`",
                f"Repeated integer: `{result.repeated_integer}`",
                f"First seen step: {result.first_seen_step}",
                f"Repeated at step: {result.repeated_at_step}",
                ...
                f"Cell: `{cell_id}`",
                f"Family: `{family}`",
                f"Generation parameters: `{json.dumps(generation_parameters, sort_keys=True)}`",
                f"Source metadata: `{json.dumps(source_metadata, sort_keys=True)}`",
                f"Validation mode: `{validation_mode}`",
                "Independent replay confirmed: true",
                "",
                "## Ordered cycle members",
                "",
                *[f"- `{v}`" for v in payload["cycle_members"]],
                "",
            ]
        )
    )
    return {
        "cycle_candidates": "results/cycle_candidates.json",
        "discovery_json": "results/nontrivial_cycle_discovery.json",
        "discovery_markdown": "NONTRIVIAL_CYCLE_FOUND.md",
    }
