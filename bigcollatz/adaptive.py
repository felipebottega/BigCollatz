...
def validate_record_for_cell(
    record: CandidateRecord, cell: AdaptiveCell, seen: set[int]
) -> None:
    validate_cell(cell)
    ...
    # The equality check above guarantees a concrete parent and also documents
    # that every adaptive candidate belongs to the configured source lineage.
    ...
    if cell.family == "parity-prefix":
        if record.prefix_length is None:
            raise ValueError("missing prefix_length")
        if record.prefix_length != cell.parameters["prefix_length"]:
            raise ValueError("mismatched prefix_length")
        if (
            record.suffix_digits is not None
            or record.residue_modulus is not None
            or record.residue is not None
        ):
            raise ValueError("unrelated metadata in parity record")
        if not validate_parity_prefix(
            record.candidate, record.parent, record.prefix_length
        ):
            raise ValueError("failed mathematical invariant")
    elif cell.family == "decimal-suffix":
        if record.suffix_digits is None:
            raise ValueError("missing suffix_digits")
        if record.suffix_digits != cell.parameters["suffix_digits"]:
            raise ValueError("mismatched suffix_digits")
        if (
            record.prefix_length is not None
            or record.residue_modulus is not None
            or record.residue is not None
        ):
            raise ValueError("unrelated metadata in suffix record")
        if not validate_decimal_suffix(
            record.candidate, record.parent, record.suffix_digits
        ):
            raise ValueError("failed mathematical invariant")
    else:
        if record.residue_modulus is None:
            raise ValueError("missing residue_modulus")
        if record.residue is None:
            raise ValueError("missing residue")
        if record.residue_modulus != cell.parameters["residue_modulus"]:
            raise ValueError("mismatched residue_modulus")
        if record.residue != cell.source_parent % record.residue_modulus:
            raise ValueError("mismatched residue")
        if record.prefix_length is not None or record.suffix_digits is not None:
            raise ValueError("unrelated metadata in residue record")
...
def _percentile(values: list[int], p: float) -> float | None:
    ...
    return (
        ordered[lo]
        if not frac
        else ordered[lo] + frac * (ordered[lo + 1] - ordered[lo])
    )
...
def _metric_aggregates(metrics: list[EvaluationMetrics]) -> dict[str, Any]:
    ...
    descents = [
        m.first_descent_step for m in metrics if m.first_descent_step is not None
    ]
    maximum = max(
        metrics,
        key=lambda m: Fraction(
            m.maximum_excursion_numerator, m.maximum_excursion_denominator
        ),
    )
    return {
        ...
        "maximum_excursion": {
            "numerator": maximum.maximum_excursion_numerator,
            "denominator": maximum.maximum_excursion_denominator,
        },
        "mean_same_decimal_digit_band_return_count": statistics.fmean(
            m.same_decimal_digit_band_return_count for m in metrics
        ),
        "mean_repeated_residue_hit_count": statistics.fmean(
            m.repeated_residue_hit_count for m in metrics
        ),
...
def _score(summary: dict[str, Any]) -> float:
    ...
    return (
        summary["mean_trajectory_length"]
        + summary["p90_trajectory_length"]
        + p99
        + 10 * summary["overall_pilot_top_tail_count"]
        + m["mean_repeated_residue_hit_count"]
    )


def rank_trajectories(
    trajectories: list[dict[str, Any]], limit: int = 30
) -> list[dict[str, Any]]:
...
def _finalize_completed_cells(
    cell_summaries: list[dict[str, Any]], trajectories: list[dict[str, Any]]
) -> None:
    ...
    for t in selected:
        tail_counts[t["cell_id"]] = tail_counts.get(t["cell_id"], 0) + 1
    for s in cell_summaries:
        lengths = s.pop("_lengths")
        s["fixed_threshold_exceedance_counts"] = {
            f"length_gte_{x}": sum(v >= x for v in lengths) for x in THRESHOLDS
        }
...
def run_adaptive_pilot(
    output_root: Path,
    *,
    pilot_id: str,
    deterministic_seed: str,
    cells: list[AdaptiveCell],
    generators: dict[str, Iterable[CandidateRecord]],
    evaluator: Callable[..., Any] = evaluate_with_metrics,
    timer: Callable[[], float] = time.perf_counter,
) -> dict[str, Any]:
    _validate_pilot_id(pilot_id)
    requested = sum(c.candidate_count for c in cells)
    seen: set[int] = set()
    cell_summaries = []
    trajectories = []
    ...
    result_dir = output_root / "results" / pilot_id
    result_dir.mkdir(parents=True, exist_ok=True)

    def global_isolated() -> bool:
        return (
            global_path.read_bytes() if global_path.exists() else None
        ) == initial_global
    ...
    def write_summary(reason: str, stopped: bool, failure: str | None = None):
        ...
        summary = {
            ...
            "distinct_candidate_count": len(seen),
            "stopped_early": stopped,
            "stopping_reason": reason,
            "outcome_counts": outcomes,
            "elapsed_runtime_seconds": timer() - start_time,
            "global_top_10_isolated": isolated,
            "cells": cell_summaries,
            "artifact_paths": artifacts,
        }
        if failure:
            summary["failure_message"] = failure
            summary["verification_failure"] = (
                failure
                if reason == "verification_failed"
                else summary.get("verification_failure")
            )
    ...
    try:
        for cell in cells:
            validate_cell(cell)
            cell_start = timer()
            iterator = iter(generators[cell.cell_id])
            lengths = []
            maxint = 0
            counts = {"reached_one": 0, "repeated_state": 0, "interrupted": 0}
            evaluated = 0
            metrics_list = []
            for requested_index in range(cell.candidate_count):
                try:
                    ...
                    raise ValueError(
                        f"short generator for cell {cell.cell_id}: expected {cell.candidate_count}, got {requested_index}"
                    ) from exc
                ...
                maxint = max(maxint, result.maximum_integer)
                ...
                trajectories.append(
                    {
                        ...
                        "source_parent": str(cell.source_parent),
                        "parent_rank": cell.parent_rank,
                        "generation_parameters": dict(cell.parameters),
                        "validation_mode": cell.validation_mode,
                        "deterministic_seed": deterministic_seed,
                        "candidate_order_within_cell": evaluated - 1,
                    }
                )
                if result.outcome == "repeated_state":
                    members = reconstruct_cycle(record.candidate, result)
                    verification = verify_nontrivial_cycle(
                        record.candidate, result, [str(v) for v in members]
                    )
                    if not verification.confirmed:
                        write_summary(
                            "verification_failed", True, verification.failure_reason
                        )
                        raise ValueError(
                            f"cycle verification failed: {verification.failure_reason}"
                        )
                    artifacts.update(
                        write_discovery_artifacts(
                            output_root,
                            starting_integer=record.candidate,
                            result=result,
                            ...
                            cell_id=cell.cell_id,
                            family=cell.family,
                            generation_parameters=cell.parameters,
                    ...
                    cell_summaries.append(
                        {
                            **_cell_dict(cell),
                            "requested_candidate_count": cell.candidate_count,
                            "candidates_evaluated": evaluated,
                            "partial": True,
                            "reached_one_count": counts["reached_one"],
                            "repeated_state_count": counts["repeated_state"],
                            ...
                            "p90_trajectory_length": _percentile(lengths, 0.9),
                            "p99_trajectory_length": None,
                            "p99_fallback_trajectory_length": max(lengths),
                            "maximum_trajectory_length": max(lengths),
                            "maximum_integer_reached": str(maxint),
                            "fixed_threshold_exceedance_counts": None,
                            "overall_pilot_top_tail_count": None,
                            "recurrence_metric_aggregates": None,
                            "deterministic_score": None,
                            "runtime_seconds": runtime,
                            "trajectories_per_second": _trajectories_per_second(
                                evaluated, runtime
                            ),
                        }
                    )
            ...
            try:
                ...
                raise ValueError(
                    f"long generator for cell {cell.cell_id}: produced more than {cell.candidate_count}"
                )
            runtime = timer() - cell_start
            if evaluated != cell.candidate_count:
            ...
            cell_summaries.append(
                {
                    **_cell_dict(cell),
                    ...
                    "median_trajectory_length": statistics.median(lengths),
                    "p90_trajectory_length": _percentile(lengths, 0.9),
                    "p99_trajectory_length": p99,
                    "p99_fallback_trajectory_length": max(lengths),
                    "maximum_trajectory_length": max(lengths),
                    "maximum_integer_reached": str(maxint),
                    "recurrence_metric_aggregates": aggs,
                    "runtime_seconds": runtime,
                    "trajectories_per_second": _trajectories_per_second(
                        evaluated, runtime
                    ),
        ...
        if total != requested or len(seen) != total:
        ...
        (result_dir / "top_30.json").write_text(
            json.dumps(rank_trajectories(trajectories, 30), indent=2, sort_keys=True)
            + "\n"
        )
        summary = write_summary("completed", False)
        summary["overall_pilot_top_tail_count"] = sum(
            s["overall_pilot_top_tail_count"] for s in cell_summaries
        )
        (result_dir / "summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n"
        )
        if not global_isolated():
            raise RuntimeError("adaptive pilot modified global_top_10.json")
        return summary
    except Exception as exc:
        if (
            isinstance(exc, RuntimeError)
            and str(exc) == "adaptive pilot modified global_top_10.json"
        ):
            raise
        if not (result_dir / "summary.json").exists():
            write_summary(type(exc).__name__, True, str(exc))
        raise
